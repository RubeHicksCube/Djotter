...
const calculateDaysSince = (date) => {
  ...
  const handleStartTimer = async (id) => {
    try {
      ...
      if (error.message && error.message.includes('locked')) {
        alert('Timer is locked. Use the reset button (🔄) to unlock it.');
      }
    }
  };
  ...
  const handleDragEnd = async (event, listType) => {
    ...
    switch (listType) {
      case 'timeSinceTrackers':
        items = state.timeSinceTrackers;
        reorderFn = api.reorderTimeSinceTrackers;
        break;
      case 'durationTrackers':
        items = state.durationTrackers;
        reorderFn = api.reorderDurationTrackers;
        break;
      case 'customCounters':
        items = state.customCounters;
        reorderFn = api.reorderCustomCounters;
        break;
      default:
        return;
    }
        ...
        <div className="card card-warning">
          ...
          {state.timeSinceTrackers.length > 0 && (
            <DndContext
              ...
              onDragEnd={(event) => handleDragEnd(event, 'timeSinceTrackers')}
            >
              <SortableContext
                items={state.timeSinceTrackers.map(t => t.id)}
                strategy={verticalListSortingStrategy}
              >
                <div className="trackers-list">
                  {state.timeSinceTrackers.map((tracker) => (
                    <SortableItem key={tracker.id} id={tracker.id}>
                      <div className="tracker-item time-since-item">
        ...
        <div className="card card-success">
          ...
          {state.durationTrackers.length > 0 && (
            <DndContext
              sensors={sensors}
              collisionDetection={closestCenter}
              onDragEnd={(event) => handleDragEnd(event, 'durationTrackers')}
            >
              <SortableContext
                items={state.durationTrackers.map(t => t.id)}
                strategy={verticalListSortingStrategy}
              >
                <div className="trackers-list">
                  {state.durationTrackers.map((tracker) => (
                    <SortableItem key={tracker.id} id={tracker.id}>
                      <div className="tracker-item duration-item">
                        <div className="tracker-info">
                          <strong>{tracker.name}</strong>
                          <span className="tracker-detail">
                            <span className={`timer-display ${tracker.isLocked ? 'locked' : ''}`}>
                              ⏱️ {calculateElapsedTime(tracker)}
                              {tracker.isLocked && <span style={{ marginLeft: '0.5rem', opacity: '0.7' }}>🔒</span>}
                            </span>
                          </span>
                        </div>
        ...
        <div className="card card-primary">
          ...
          {state.customCounters && state.customCounters.length > 0 && (
            <DndContext
              sensors={sensors}
              collisionDetection={closestCenter}
              onDragEnd={(event) => handleDragEnd(event, 'customCounters')}
            >
              <SortableContext
                items={state.customCounters.map(c => c.id)}
                strategy={verticalListSortingStrategy}
              >
                <div className="trackers-list">
                  {state.customCounters.map((counter) => (
                    <SortableItem key={counter.id} id={counter.id}>
                      <div className="tracker-item counter-item">
                        <div className="tracker-info">
