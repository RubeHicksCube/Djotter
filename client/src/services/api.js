...
export const api = {
  ...
  getStateForDate: async (date) => {
    const response = await fetch(`${API_BASE}/state/${date}`, {
      headers: getAuthHeaders()
    });
    if (response.status === 401) {
      window.location.href = '/login';
  ...
  // Add activity entry
  addEntry: async (text, image = null, date = null) => {
    const response = await fetch(`${API_BASE}/entry`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({ text, image, date })
    });
    return response.json();
  },
  ...
  toggleTaskPinned: async (id) => {
    const response = await fetch(`${API_BASE}/daily-tasks/${id}/toggle-pinned`, {
      method: 'PUT',
      headers: getAuthHeaders()
    });
    return response.json();
  ...
  toggleTaskRecurring: async (id) => {
    const response = await fetch(`${API_BASE}/daily-tasks/${id}/toggle-recurring`, {
      method: 'PUT',
      headers: getAuthHeaders()
    });
    return response.json();
  ...
  createCustomFieldTemplate: async (key, fieldType = 'text') => {
    const response = await fetch(`${API_BASE}/custom-field-templates/create`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({ key, fieldType })
    });
    return response.json();
  },
  ...
  updateCustomFieldValue: async (key, data) => {
    // Handle both old format (value) and new format (object with value and date)
    const payload = typeof data === 'object' ? data : { value: data };
  ...
  updateCustomFieldType: async (id, fieldType) => {
    const response = await fetch(`${API_BASE}/custom-field-templates/${id}`, {
      method: 'PUT',
      headers: getAuthHeaders(),
  ...
  searchEntries: async (searchText, searchDate = null) => {
    const response = await fetch(`${API_BASE}/exports/search-entries`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({ searchText, searchDate })
    });
    if (!response.ok) {
  ...
  // Reordering
  reorderCustomFieldTemplates: async (items) => {
    const response = await fetch(`${API_BASE}/reorder/custom-field-templates`, {
      method: 'POST',
      headers: getAuthHeaders(),
  ...
  reorderTimeSinceTrackers: async (items) => {
    const response = await fetch(`${API_BASE}/reorder/time-since-trackers`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({ items })
    });
  ...
  reorderDurationTrackers: async (items) => {
    const response = await fetch(`${API_BASE}/reorder/duration-trackers`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({ items })
    });
  ...
  reorderCustomCounters: async (items) => {
    const response = await fetch(`${API_BASE}/reorder/custom-counters`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({ items })
    });
  ...
  reorderDailyCustomFields: async (items) => {
    const response = await fetch(`${API_BASE}/reorder/daily-custom-fields`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({ items })
    });
  ...
  reorderDailyTasks: async (items) => {
    const response = await fetch(`${API_BASE}/reorder/daily-tasks`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({ items })
    });
  ...
  reorderActivityEntries: async (items) => {
    const response = await fetch(`${API_BASE}/reorder/activity-entries`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({ items })
    });
  ...
  queryTasks: async (params) => {
    const response = await fetch(`${API_BASE}/queries/tasks`, {
      method: 'POST',
      headers: getAuthHeaders(),
    ...
    if (!response.ok) {
      ...
      throw new Error(error.error || 'Failed to query tasks');
    }
    return response.json();
  },

  queryFields: async (params) => {
    const response = await fetch(`${API_BASE}/queries/fields`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify(params)
    });
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'Failed to query fields');
    }
    return response.json();
  },
  ...
  exportCSV: async (type, queryParams) => {
    const response = await fetch(`${API_BASE}/exports/csv`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({ type, queryParams })
    });

    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'Failed to export to CSV');
    }
  ...
  setManualTime: async (trackerId, startTime, elapsedMs) => {
    const response = await fetch(`${API_BASE}/trackers/manual-time`, {
      method: 'POST',
      headers: getAuthHeaders(),
